...
namespace tttgame
{
    class Program
    {
        ...
        private static bool IsGameMarkersTheSame(char[] testGameMakers, int pos1, int pos2, int pos3)
        {
            return testGameMakers[pos1].Equals(testGameMakers[pos2]) && testGameMakers[pos2].Equals(testGameMakers[pos3]);
        }
        ...
        private static void GameEngine(char[] gameMarkers, int currentPlayer)
        {
            ...
            do
            {
                    ...
                    userInput.Equals("9")))
                {
                    ...
                    char currentMarker = gameMarkers[gamePlaceMarker - 1];

                    if (currentMarker.Equals('X') || currentMarker.Equals('O'))
                    {
                        Console.WriteLine("Placement already has a marker. Please select another placement.");
                    }
                    else
                    {
                        gameMarkers[gamePlaceMarker - 1] = GetPlayerMarker(currentPlayer);
        ...
        static void HeadsUpDisplay(int PlayerNumber)
        {
            ...
            //1.2  Display players sign, Player 1 is X and players 2 is O 
